...
[[ ! -e "$TEST_HOME/.codex/codex-workbench/hooks/codex-workbench-lifecycle.py" ]] \
    || { echo "FAIL: dry-run 安装了 collector" >&2; exit 1; }

python3 "$CONFIGURATOR" --home "$TEST_HOME" --install >/dev/null

collector="$TEST_HOME/.codex/codex-workbench/hooks/codex-workbench-lifecycle.py"
[[ -x "$collector" ]] \
...
mkdir -p "$CONFLICT_HOME/.codex/codex-workbench/hooks/codex-workbench-lifecycle.py"
